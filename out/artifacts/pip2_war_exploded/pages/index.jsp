...
<jsp:useBean id="history" class="bean.UserHistory" scope="session"/>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<script type="text/javascript" charset="UTF-8"><%@include file='functions.js' %></script>
...
<div id="content">
  <div id="smth">
    <div id="changes">
      ...
      <input type="submit"  value="Проверить" name="submit_form" onclick="showResults()">
        <input type="button" value="Очистить" onclick="cleanAll()">
    </div>
    <div id="graf">
      <svg id="grafic" >
          <rect id="odz" height="240" width="300" x="0" y="30" fill="none"></rect>
    ...
    <div class="results" id="results">
        ...
        <h1>History</h1><button type="button" onclick="clearHistory(); location.reload()" id="history-button" class="button history-button">Clean history</button><br>
        <table id="result-table">
            <tr id="table-headers"><th>X</th><th>Y</th><th>R</th><th>Result</th><th>Time of request</th></tr>
            <%
                List<PointBean> list = new ArrayList<PointBean>(history.getListOfPoint());
                Collections.reverse(list);
                for (PointBean p : list){
                    System.out.println(p.toString());%>
            <tr><td class="x"><%=p.getX()%></td><td class="y"><%=p.getY()%></td><td class="r"><%=p.getR()%></td><td><%=p.isInArea()%></td><td><%=p.getDate()%></td></tr>
            <%}%>
        </table>
        <%}%>
    </div>
